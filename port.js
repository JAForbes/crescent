...
module.exports = function() {
  ...
  tableAccess = function(tablename, user) {
    ...
    if (role === 'admin') {
      ...
      _(result).each(function(row, id) {
        if (__indexOf.call(row.access, user) >= 0) {
  ...
  rowAccess = function(row, user) {
    ...
    role = database.data[user].role;
    allowed = role === 'admin' || role === 'user' && __indexOf.call(result.access, user) >= 0;
    if (allowed) {
      return result;
    } else {
  ...
  apply = function(tablename, user, apply) {
    ...
    _(tableAccess(tablename, user)).each(function(row, id) {
      ...
      if (!changed) {
        return changed = !_.isEqual(row, before);
      }
    });
    done = function() {
      return function(query) {
  ...
  save = function(done, args) {
    _.defer(function() {
      return fs.writeFile('./db/data.json', JSON.stringify(database, null, 2), "utf8");
    });
